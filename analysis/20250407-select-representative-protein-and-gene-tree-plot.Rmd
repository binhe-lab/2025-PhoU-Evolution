...
First import the species tree
```{r}
gene_tree <- read.tree("../output/gene-tree/20250426-PhoU-PitR-select-raxml-PROTGAMMA-LG-best-ML-tree.nwk") %>% 
  as_tibble() %>% 
...
plot the tree
```{r}
p.tree <- ggtree(gene_tree, layout = "equal_angle") + #xlim(0,2.2) + scale_y_reverse() +
...
p.tree
ggsave("../output/gene-tree/20250426-PhoU-PitR-select-raxml-PROTGAMMA-LG-best-ML-tree.pdf", 
       p.tree, width = 8, height = 8)
```
...
```{r}
gene_tree_2 <- read.tree("../output/gene-tree/20250426-PhoU-PitR-select-raxml-PROTCAT-Dayhoff-best-ML-tree.nwk") %>% 
  as_tibble() %>% 
  #mutate(label = gsub("_", " ", label)) %>% 
  left_join(seq_for_tree, by = c("label" = "Accession")) %>% 
  as.treedata()
...
plot the tree
```{r}
p.tree.2 <- ggtree(gene_tree_2, layout = "equal_angle") + #xlim(0,2.2) + scale_y_reverse() +
  geom_tiplab(aes(subset = label %in% c("A0A0H3K808", "A0A0H3K7D8"), 
                  angle = angle, label = paste("S. aureus", group, sep = " ")), 
              size = 3.5, geom = "label") +
  #geom_treescale(x = 0, width = 0.2, linesize = 1.2) +
  ...
  theme(legend.position = c(0.12, 0.13),
        ...
        legend.text = element_text(size = rel(1.2)))
p.tree.2
ggsave("../output/gene-tree/20250426-PhoU-PitR-select-raxml-PROTCAT-Dayhoff-best-ML-tree.pdf", 
       p.tree.2, width = 8, height = 8)
```